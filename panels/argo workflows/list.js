...
export async function load(args) {
    ...
    const [workflows, podMetrics] = await runJsonCommands(["list-workflows", "pod-metrics"])

    const data = loadWorkflows(workflows, podMetrics, args)
    setData([data])
}
 ...
 * @param args {LoadFunctionArgs}
 */
export function loadWorkflows(allPods, allPodMetrics, args) {
    ...
    function* extractData() {
        ...
        for (const pod of allPods.items) {
            ...
            const { startedAt, finishedAt, phase } = status
            ...
            if (show === undefined) {
                error("unknown workflow phase", phase)
            }
